...
link_or_copy_file "${CONTROL_PLANE_IMAGE}" "${RELEASE_INPUT_DIR}/${CONTROL_PLANE_BASENAME}"
link_or_copy_file "${UI_IMAGE}" "${RELEASE_INPUT_DIR}/${UI_BASENAME}"
if [[ -n "${HOST_AGENT_IMAGE}" ]]; then
  HOST_AGENT_IMAGE_BASENAME="$(basename "${HOST_AGENT_IMAGE}")"
  link_or_copy_file "${HOST_AGENT_IMAGE}" "${RELEASE_INPUT_DIR}/${HOST_AGENT_IMAGE_BASENAME}"
fi
...
if [[ -n "${MESSAGE_BROKER_IMAGE}" ]]; then link_or_copy_file "${MESSAGE_BROKER_IMAGE}" "${RELEASE_INPUT_DIR}/${MESSAGE_BROKER_BASENAME}"; fi
...
if [[ -n "${ARTIFACT_SERVER_IMAGE}" ]]; then
  ...
  link_or_copy_file "${ARTIFACT_SERVER_IMAGE}" "${RELEASE_INPUT_DIR}/${ARTIFACT_SERVER_BASENAME}"
fi
if [[ -n "${DNS_IMAGE}" ]]; then
  DNS_BASENAME="$(basename "${DNS_IMAGE}")"
  link_or_copy_file "${DNS_IMAGE}" "${RELEASE_INPUT_DIR}/${DNS_BASENAME}"
fi
BLOB_STORAGE_BASENAME="$(basename "${BLOB_STORAGE_IMAGE}")"
link_or_copy_file "${BLOB_STORAGE_IMAGE}" "${RELEASE_INPUT_DIR}/${BLOB_STORAGE_BASENAME}"
if [[ -n "${INFERENCE_RUNTIME_IMAGE}" ]]; then
  INFERENCE_RUNTIME_BASENAME="$(basename "${INFERENCE_RUNTIME_IMAGE}")"
  link_or_copy_file "${INFERENCE_RUNTIME_IMAGE}" "${RELEASE_INPUT_DIR}/${INFERENCE_RUNTIME_BASENAME}"
fi
...
if [[ -n "${WORKFLOW_CONTROLLER_IMAGE}" ]]; then
  ...
  link_or_copy_file "${WORKFLOW_CONTROLLER_IMAGE}" "${RELEASE_INPUT_DIR}/${WORKFLOW_CONTROLLER_BASENAME}"
fi
if [[ -n "${WORKFLOW_EXECUTOR_IMAGE}" ]]; then
  WORKFLOW_EXECUTOR_BASENAME="$(basename "${WORKFLOW_EXECUTOR_IMAGE}")"
  link_or_copy_file "${WORKFLOW_EXECUTOR_IMAGE}" "${RELEASE_INPUT_DIR}/${WORKFLOW_EXECUTOR_BASENAME}"
fi
if [[ ${#EXTRA_OCI_IMAGES[@]} -gt 0 ]]; then
  for extra_image in "${EXTRA_OCI_IMAGES[@]}"; do
    extra_basename="$(basename "${extra_image}")"
    EXTRA_OCI_BASENAMES+=("${extra_basename}")
    link_or_copy_file "${extra_image}" "${RELEASE_INPUT_DIR}/${extra_basename}"
  done
fi
