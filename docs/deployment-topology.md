...
This document is the visual reference for the appliance's production K3s layout. It distinguishes always-running product pods, the workflow engine, temporary workflow task pods, K3s-managed platform pods, storage, and external systems.
...
flowchart TD
    ...
    subgraph Node["Dedicated single-node K3s appliance"]
        ...
        subgraph WorkflowNS["workflows namespace"]
            WorkflowController["Workflow Controller pod<br/>Internal workflow reconciliation"]
        end

        subgraph BuildNS["appliance-builds namespace"]
            Build["Ephemeral Buildah task pod<br/>Created by a Workflow"]
    ...
    Traefik -->|"/v2/*"| ArtifactServer

    Control --> ControlPVC
    ArtifactServer --> ArtifactServerPVC
    Control -->|"Submit, observe, and terminate constrained Workflows"| K3sAPI
    WorkflowController -->|"Watch Workflows and reconcile task pods"| K3sAPI
    K3sAPI -->|"Workflow status"| Control
    WorkflowController -.->|"Creates and supervises"| Build
    WorkflowController -.->|"Creates and supervises"| Scan
    WorkflowController -.->|"Creates and supervises"| ImageOps

    Build -->|"Fetch immutable commit"| Source
    Build -->|"Publish OCI image"| ArtifactServer
    Scan -->|"Read image by digest"| ArtifactServer
    ImageOps -->|"OCI image/artifact operations"| ArtifactServer

    Backup --> ControlPVC
    Backup --> ArtifactServerPVC
    Backup --> BackupTarget
    Upgrade --> ControlPVC
    Upgrade --> ArtifactServerPVC
```
...
| --- | --- | --- | --- |
| Buildah build | One appliance-generated Workflow per build | Until success, failure, cancellation, or deadline | Rootless, non-privileged task pod, no host runtime socket, `emptyDir` workspace, short-lived credentials, default-deny network policy, resource and log limits |
...
| Backup | Kubernetes CronJob plus on-demand operator run | Scheduled maintenance window | Quiesces relevant background work, captures consistent SQLite/artifact-server state, encrypts and exports off appliance, verifies checksums |
...
The control-plane server does not run arbitrary shell commands in HTTP handlers and does not accept arbitrary user-supplied Workflow YAML. It renders versioned, allowlisted workflow templates through a typed adapter. The workflow engine owns task scheduling and pod reconciliation; the control plane owns authentication, authorization, business state, audit, cancellation intent, and result acceptance.

Completed Workflow resources have a bounded TTL. The control-plane SQLite database is authoritative for durable build/workflow history, so v1 does not enable workflow archive/offloading or add a separate database for the workflow engine.
...
| Workflow Controller | Kubernetes API | Reconcile Workflow CRs and their task pods in the managed build namespace |
| Control plane | Artifact Server | Health, catalog reconciliation, and authorized lifecycle operations |
| Build task pod | Allowlisted internal Git hosts | Fetch immutable source commit |
| Build task pod | Artifact Server | Pull approved base images and push only to authorized repository prefixes |
| Scan/utility task pods | Artifact Server | Digest-bound read or explicitly authorized copy/write operations |
...
flowchart LR
    ...
    ArtifactServer["Artifact Server pod"] --> ZPVC[("RWO PVC")]
...
The Artifact Server content and control-plane metadata have different authorities:

- Artifact Server is authoritative for OCI manifests, tags, digests, referrers, and blobs.
...
- Workflow CRs and task pods are operational state. Versioned workflow templates are release assets, while durable operation history and accepted results live in SQLite.
...
- Argo Server/UI or a workflow-archive database
...
K3s platform pods
  ...
  + one Artifact Server pod
  + one Workflow Controller pod
  + their two RWO PVCs
```

Build, scan, and image-operation task pods exist only while their Workflows are active. Backup, upgrade, and restore pods exist only while their corresponding operational Jobs are active. V1 ships this complete topology as one appliance package; package profiles are deferred.
