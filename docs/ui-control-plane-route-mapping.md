...
estimated RAM). The selector also shows non-fitting catalog candidates with the
manager's reason and disables their Download action rather than hiding them.
It initially selects an eligible candidate when one exists. Enabling a downloaded model replaces the currently enabled
model.
...
| `GET /api/v1/inference/models/catalog?sort=parameters&order=desc` | `getInferenceCatalog` | Cached candidates ordered by estimated parameter scale (default); also supports `sort=memory|name` and `order=asc|desc`. The manager starts Ollama discovery on every process start with a separate two-minute network deadline; this read never starts discovery or waits for the engine. The saved list is served immediately. While `refreshing` is true, the AI Services page re-reads the cache every three seconds and displays candidates when they arrive. Slow or failed metadata requests retain healthy candidates and schedule a 15-minute retry. Tool-aware candidates are classified from their published template layer; when that template is unavailable, the candidate remains visible as Capability unverified instead of being mislabeled Chat assistant. Older v2/v3 caches are preserved through offline upgrade; the Ollama memory estimate is recalculated from cached model-layer weights, and v2 capability labels remain unverified until refreshed. The selector includes ineligible entries and displays their reason. `runtimeVersion` identifies the packaged runtime associated with the cached catalog. |
| `GET /api/v1/inference/models` | `listInferenceModels` | Actual downloaded inventory, including appliance-declared experiences and client compatibility. For Ollama, the inference manager loads the persisted inventory before opening its API, then refreshes tags (including local model `sizeBytes`, passed through the control plane) and `/api/show` in the background; import/delete also refresh it. A failed `/api/show` retains a prior verified capability or marks a new model unverified; it never asserts Chat assistant from an error. On a first-run empty inventory, the UI retries at 2-second intervals (up to 10 seconds) before returning to its normal refresh interval. The UI uses these values to distinguish chat-only from coding-agent models and only offers Codex settings for compatible models. |
...
| `GET /api/v1/inference/models/imports/progress` | `getInferenceImportProgress` | Bytes, phase, and state for the active or most recent import; polled about every 2 seconds while downloading. Ollama pulls stream progress events into this response and report a failed state if the stream stops making progress for two minutes. |
...
| `GET /admin/host-services/mdns` | `AdminHostServicesPage` mDNS tab | `GET /api/v1/host/mdns` | mDNS status card, including advertised `<appliance-name>.local` when available |
| Enable or Disable mDNS | `applyHostMDNS` | `PUT /api/v1/host/mdns` with `{desired}` | One action button from status (`desired`/`actual`); busy label while apply runs; card refresh with the stored appliance mDNS name |
