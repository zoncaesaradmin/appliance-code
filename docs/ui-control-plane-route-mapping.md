...
| `GET /manage/files` | React `FilesPage` | `GET /api/v1/files` (and `GET /api/v1/files/{path}` when browsing a directory); upload dialog `POST`s octet-stream to `/api/v1/files/{logical-path}`; download uses `GET /api/v1/files/{path}` as a blob | SPA page |
