...
| `GET /manage/videos` | React `VideosPage` | Capability-gated (`video`). `GET /api/v1/video/library` (and path variants); upload `POST /api/v1/video/library/{path}`; play fetches blob via `GET` then HTML5 `<video>`; delete `DELETE /api/v1/video/library/{path}` | SPA page; library is `/data/zon/video/library` on the host |
