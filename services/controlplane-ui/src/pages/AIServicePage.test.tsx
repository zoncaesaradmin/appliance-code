...
const api = vi.hoisted(() => ({ getInferenceStatus: vi.fn(), listInferenceModels: vi.fn(), getInferenceCatalog: vi.fn(), importInferenceModel: vi.fn(), loadInferenceModel: vi.fn(), deleteInferenceModel: vi.fn() }));
vi.mock("../lib/api", () => ({ client: api }));
vi.mock("../lib/navigate", () => ({ navigate: vi.fn() }));
vi.mock("../components", () => ({
  PageFrame: ({ children }: {children: React.ReactNode}) => <main>{children}</main>,
  Card: ({ title, children }: {title: string; children: React.ReactNode}) => <section><h2>{title}</h2>{children}</section>,
...
beforeEach(() => {
  ...
  api.getInferenceCatalog.mockResolvedValue({ lastSuccess: "2026-09-15T00:00:00Z", stale: true, refreshing: false, scope: "Popular models", items: [
...
it("keeps downloaded models separate from eligible candidates and selects without freeform input", async () => {
  await act(async () => root.render(<AIServicePage />));
  expect(element.textContent).toContain("retired:1b");
  expect(element.textContent).toContain("✓ Downloaded");
  expect(element.textContent).toContain("available:1b");
  expect(element.textContent).not.toContain("too-large:100b");
  expect(element.querySelector("textarea")).toBeNull();
  const download = [...element.querySelectorAll("button")].find((button) => button.textContent === "Download now")!;
  await act(async () => download.click());
  expect(api.importInferenceModel).toHaveBeenCalledWith({ catalogId: "available:1b", modelId: "available:1b", source: "available:1b" });
});

it("shows downloaded models even when catalog discovery is unavailable", async () => {
