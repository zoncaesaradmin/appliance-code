...
func TestSubmitCreatesWorkspaceLocalBuildWorkflow(t *testing.T) {
	got, err := workflowObject("appliance-builds", "", "", workflows.Spec{
		...
		WorkspaceClaimName: "controlplane-workspaces",
		WorkspaceName:      "demo",
	...
	for _, want := range []string{
		`mkdir -p "$HOME" "$GOPATH" "$GOCACHE" "$GOMODCACHE" "$GOTMPDIR" "$XDG_CACHE_HOME"`,
...
func TestSubmitCreatesBuildWorkflowWithSharedWorkspaceMount(t *testing.T) {
	...
	text := workflowJSON(t, got)
	for _, want := range []string{"workspace-storage", "controlplane-workspaces", "WORKSPACE_ROOT_DIR", "/data/zon/workspaces"} {
		if !strings.Contains(text, want) {
...
func TestSubmitCreatesWorkspacePrepareWorkflow(t *testing.T) {
	...
	text := workflowJSON(t, got)
	command := workflowCommand(t, got)
	for _, want := range []string{"workspace-storage", "controlplane-workspaces", "WORKSPACE_ROOT_DIR", "WORKSPACE_NAME", "platformkit", "forgeline", "git-access-0", "git-access-git-internal", "/var/run/appliance/git-access/git-internal"} {
		if !strings.Contains(text, want) {
			t.Fatalf("workspace workflow JSON missing %q: %s", want, text)
		}
	}
	for _, want := range []string{"podSpecPatch", "\\\"name\\\":\\\"init\\\"", "\\\"name\\\":\\\"wait\\\"", "RuntimeDefault", "runAsNonRoot", "runAsUser", "10010", "fsGroup", "20000", "OnRootMismatch"} {
		if !strings.Contains(text, want) {
			t.Fatalf("workspace workflow JSON missing %q: %s", want, text)
		}
	}
...
func TestSubmitWorkspacePrepareRequiresGitCredential(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "builder Git HTTPS credentials") {
		t.Fatalf("expected workspace prepare without Git credentials to fail, got %v", err)
	}
}
