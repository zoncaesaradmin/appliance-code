...
func TestInferenceRuntimeUsesSharedGateway(t *testing.T) {
	...
	if module.BaseURL != "http://inference-gateway.inference.svc.cluster.local:8080" || len(module.Routes) != 1 {
		t.Fatalf("inference gateway contract changed: %+v", module)
	}
	route := module.Routes[0]
	if route.Method != "ANY" ||
		route.ExternalPath != "/inference/{path...}" ||
		route.StripPrefix != "/inference" ||
		route.UpstreamPath != "" ||
		route.Permission != "inference.use" {
		t.Fatalf("inference OpenAI catch-all route changed: %+v", route)
	}
}
