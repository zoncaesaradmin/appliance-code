...
func TestLoadAppliesApplianceCatalogJSON(t *testing.T) {
	catalogJSON := `{"version":"appliance.catalog/v1alpha1","profiles":[{"name":"custom","capabilities":["base","host"]}],"modules":[{"name":"host-agent","kind":"platform","requiredCapabilities":["host"],"executionMode":"host-agent","entitlementKey":"host-agent","baseURL":"http://host-agent.ace-system.svc.cluster.local:8080","securityClass":"host-privileged"}]}`
	cfg, err := config.Load([]string{
